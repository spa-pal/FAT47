...
//	if(!(bps[NUMB]._cnt==CNT_SRC_MAX)) blink(0,0,0,0); //не гасить кр светодиод по потере кан
	net_av=1;
	bps[NUMB]._av&=~(0x17);// для перезапуска по другим авариям
...
//	av_bps[bps]=0;
	...
	if(v==0)
		{
		net_av=0;
		bps[NUMB]._av&=~(0x3F);// для перезапуска по другим авариям
		bps[NUMB]._overload_av_cnt=0;	   // аварии остались, то перезапустятся
		bps[NUMB]._light_overload_av_cnt=0;
		bps[NUMB]._temp_av_cnt=0;
		bps[NUMB]._umax_av_cnt=0;
		bps[NUMB]._umin_av_cnt=0;
